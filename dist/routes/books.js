...
router.get('/year/:year', (req, res, next) => {
    Book_1.default.find({ 'belongs_to_year': req.params.year }).sort('createdAt').find(function (err, books) {
        if (err)
            return next(err);
        res.json(books);
    });
...
router.put('/:id', passport_1.default.authenticate('jwt', { session: false }), (req, res, next) => {
    Book_1.default.findByIdAndUpdate(req.params.id, req.body, { runValidators: true }, (err, book) => {
        if (err)
            return next(err);
        res.json(book);
    });
...
router.delete('/:id', passport_1.default.authenticate('jwt', { session: false }), (req, res, next) => {
    Book_1.default.findByIdAndRemove(req.params.id, req.body, (err, book) => {
        if (err)
            return next(err);
        res.json(book);
    });
